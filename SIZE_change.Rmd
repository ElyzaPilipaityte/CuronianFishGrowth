...
data2023 <- read_csv(file = "lmer2023.csv")
olddata <- read_csv(file = "perchLmer.csv")

perch23 <- data2023 %>% filter (spp_name_lt=="Eserys")

perch <- rbind(olddata, perch23) %>% drop_na(age) %>% drop_na(total_length)
...
perch1 <- perch %>% filter (age < 9 & age > 1) %>% drop_na(season)
table(perch1$year)
...
temp <- read_csv(file = "temp.csv")
data2023 <- read_csv(file = "lmer2023.csv")
olddata <- read_csv(file = "breamLmer.csv")

bream23 <- data2023 %>% filter (spp_name_lt=="Karsis")

bream <- rbind(olddata, bream23) %>% drop_na(age) %>% drop_na(std_length)
...
bream1 <- bream %>% filter (age > 2 & age <= 11 & age != 5) %>% filter (std_length<55) %>% drop_na (season)
table(bream1$year)
...
temp <- read_csv(file = "temp.csv")
data2023 <- read_csv(file = "lmer2023.csv")
olddata <- read_csv(file = "roachLmer.csv")

roach23 <- data2023 %>% filter (spp_name_lt=="Kuoja")

roach <- rbind(olddata, roach23) %>% drop_na(age) %>% drop_na(std_length)
...
roach1 <- roach %>% filter (age > 2 & age <= 11) %>% drop_na(season)
table(roach1$year)
...
temp <- read_csv(file = "temp.csv")
data2023 <- read_csv(file = "lmer2023.csv")
olddata <- read_csv(file = "sanderLmer.csv")

sander23 <- data2023 %>% filter (spp_name_lt=="Starkis")

sander <- rbind(olddata, sander23) %>% drop_na(age) %>% drop_na(std_length)
...
One plot with the same x-axis range and labels for all subplots, starting from 0 on the y-axis.?? 
```{r}
library(cowplot)
...
create_plot <- function(model, species_name, show_x_axis_text = TRUE) {
  plot <- plot_model(model, type = "pred", terms = c("temp_life", "age"), show.data = TRUE, pred.type = "fe") +
    ylim(0, NA) +
    scale_x_continuous(limits = c(8, 10), breaks = c(8, 9, 10)) + 
    labs(x = NULL, y = NULL) +  # Remove x and y axis labels from individual plots
    ggtitle(species_name) +
    theme_bw() +
    theme(
