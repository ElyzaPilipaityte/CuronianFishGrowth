...
vbFitPlot1 <- ggplot() +
  ...
  scale_color_manual(values=c('#000000', '#93003a'), aesthetics=c("color", "fill")) +
...
vbFitPlot2 <- ggplot() +
  ...
  scale_y_continuous(name="Total Length (mm)", limits=c(0, 60)) +
  scale_x_continuous(name="Age (years)", breaks=0:20) +
  scale_color_manual(values=c('black', '#93003a'), aesthetics=c("color", "fill")) +
  scale_y_continuous(name=element_blank(), limits=c(0, 70), expand=c(0, 0)) +
  scale_x_continuous(name=element_blank(), expand=c(0, 0), limits=c(0, 20), breaks=seq(5, 15, 5)) +
  theme_bw() +
  theme(panel.grid=element_blank(),
...
combined_plot <- grid.arrange(
  arrangeGrob(textGrob(species_names[1], gp = gpar(fontsize = 17)), vbFitPlot1, ncol = 1, heights = c(0.1, 1)),
  arrangeGrob(textGrob(species_names[2], gp = gpar(fontsize = 17)), vbFitPlot2, ncol = 1, heights = c(0.1, 1)),
  arrangeGrob(textGrob(species_names[3], gp = gpar(fontsize = 17)), vbFitPlot3, ncol = 1, heights = c(0.1, 1)),
  arrangeGrob(textGrob(species_names[4], gp = gpar(fontsize = 17)), vbFitPlot4, ncol = 1, heights = c(0.1, 1)),
  ncol = 2, nrow = 2
)
...
# Save using ggsave via a temporary file device
tiff("vonB.tiff", units = "mm", width = 300, height = 250, res = 600, compression = "lzw")
grid.draw(full_plot)
dev.off()
```
...
prediction_data <- models %>%
  mutate(predictions = map(model, ~ data.frame(
    temp = seq(8, 10, by = 0.1),
    fish_weight = predict(.x, newdata = data.frame(temp = seq(8, 10, by = 0.1)))
  ))) %>%
