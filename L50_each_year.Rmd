...
breamMat <- slice(breamMatSize, -2, -3, -4, -7)
...
Y1959 <- runif(100, min=breamMat[2,3], max=breamMat[2,4])
Y1960 <- runif(100, min=breamMat[3,3], max=breamMat[3,4])
Y1966 <- runif(100, min=breamMat[4,3], max=breamMat[4,4])
Y1967 <- runif(100, min=breamMat[5,3], max=breamMat[5,4])
Y1968 <- runif(100, min=breamMat[6,3], max=breamMat[6,4])
Y1973 <- runif(100, min=breamMat[7,3], max=breamMat[7,4])
Y1974 <- runif(100, min=breamMat[8,3], max=breamMat[8,4])
Y1975 <- runif(100, min=breamMat[9,3], max=breamMat[9,4])
Y1976 <- runif(100, min=breamMat[10,3], max=breamMat[10,4])
Y1977 <- runif(100, min=breamMat[11,3], max=breamMat[11,4])
Y1978 <- runif(100, min=breamMat[12,3], max=breamMat[12,4])
Y1979 <- runif(100, min=breamMat[13,3], max=breamMat[13,4])
Y1984 <- runif(100, min=breamMat[14,3], max=breamMat[14,4])
Y1987 <- runif(100, min=breamMat[15,3], max=breamMat[15,4])
Y1988 <- runif(100, min=breamMat[16,3], max=breamMat[16,4])
Y1989 <- runif(100, min=breamMat[17,3], max=breamMat[17,4])
Y1990 <- runif(100, min=breamMat[18,3], max=breamMat[18,4])
Y1991 <- runif(100, min=breamMat[19,3], max=breamMat[19,4])
Y1992 <- runif(100, min=breamMat[20,3], max=breamMat[20,4])
Y1993 <- runif(100, min=breamMat[21,3], max=breamMat[21,4])
Y1995 <- runif(100, min=breamMat[22,3], max=breamMat[22,4])
Y1998 <- runif(100, min=breamMat[23,3], max=breamMat[23,4])
Y1999 <- runif(100, min=breamMat[24,3], max=breamMat[24,4])
Y2004 <- runif(100, min=breamMat[25,3], max=breamMat[25,4])
Y2006 <- runif(100, min=breamMat[26,3], max=breamMat[26,4])

# Combine the vectors into a data frame
data <- data.frame(
  Y1950, Y1959, Y1960, Y1966, Y1967, Y1968, Y1973, Y1974, Y1975, 
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -8, size = 5, color = "black") + expand_limits(y = 0) + My_theme
```
...
# Removing years where there is less than 2 mature fish (1962, 1973, 2013)
roachMat <- slice(roachMatSize, -4, -7, -13)
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 399.29488   61.06042   6.539 6.56e-05 ***
# year         -0.19077    0.03044  -6.266 9.31e-05 ***
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 494.94231  151.32682   3.271  0.00746 **
# year         -0.23830    0.07612  -3.131  0.00957 **
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -11, size = 5, color = "black") + expand_limits(y = 0) +
   My_theme
```
...
# Excluding years (1966, 1967, 1973, 1992, 2002) which have 1 or less mature fish data
sanderMat <- slice(sanderMatSize, -3, -4, -6, -13, -17)
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 986.6694   240.6056   4.101  0.00214 **
# year         -0.4774     0.1211  -3.943  0.00276 **
