...
My_theme <- theme(
  ...
  text = element_text(size = 30, family = "gillsans"),
...
br <- ggplot() +
  geom_point(data = breamMat, aes(x = year, y = predicted_values), color = "azure4", size = 1) +  # Points for predicted_values
  ...
  geom_line(data = plot_data, aes(x = x, y = predicted), color = "black") +  # Fitted line from model predictions
  geom_ribbon(data = plot_data, aes(x = x, ymin = ci.l, ymax = ci.u), alpha = 0.2, fill = "black") +
...
# Display the results
print(results_df)
medians <- apply(results_df, 2, median)
...
lmb <- ggplot(combined_sampled_data, aes(x = year, y = predicted_values.x, group = simulation)) +
 ...
 geom_line(aes(y = predicted, color = p_value < 0.05), alpha = 0.3) +
  scale_color_manual(values = c("TRUE" = "#93003a", "FALSE" = "#000000"), guide = FALSE) +
...
# year        -0.3082     0.0403  -7.647 1.74e-05 ***
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -16.5, size = 5, color = "black") + expand_limits(y = 0) + My_theme
...
L50.66p = apply(bcL3$t[,1:2] + bcL3$t[,c(3, 19)], 1, lrPerc, p = 0.5)
L50.67p = apply(bcL3$t[,1:2] + bcL3$t[,c(4, 20)], 1, lrPerc, p = 0.5)
L50.68p = apply(bcL3$t[,1:2] + bcL3$t[,c(5, 21)], 1, lrPerc, p = 0.5)
L50.73p = apply(bcL3$t[,1:2] + bcL3$t[,c(6, 22)], 1, lrPerc, p = 0.5)
L50.76p = apply(bcL3$t[,1:2] + bcL3$t[,c(7, 23)], 1, lrPerc, p = 0.5)
L50.77p = apply(bcL3$t[,1:2] + bcL3$t[,c(8, 24)], 1, lrPerc, p = 0.5)
L50.78p = apply(bcL3$t[,1:2] + bcL3$t[,c(9, 25)], 1, lrPerc, p = 0.5)
L50.79p = apply(bcL3$t[,1:2] + bcL3$t[,c(10, 26)], 1, lrPerc, p = 0.5)
L50.81p = apply(bcL3$t[,1:2] + bcL3$t[,c(11, 27)], 1, lrPerc, p = 0.5)
L50.90p = apply(bcL3$t[,1:2] + bcL3$t[,c(12, 28)], 1, lrPerc, p = 0.5)
L50.91p = apply(bcL3$t[,1:2] + bcL3$t[,c(13, 29)], 1, lrPerc, p = 0.5)
L50.09p = apply(bcL3$t[,1:2] + bcL3$t[,c(14, 30)], 1, lrPerc, p = 0.5)
L50.13p = apply(bcL3$t[,1:2] + bcL3$t[,c(15, 31)], 1, lrPerc, p = 0.5)
L50.14p = apply(bcL3$t[,1:2] + bcL3$t[,c(16, 32)], 1, lrPerc, p = 0.5)
L50.15p = apply(bcL3$t[,1:2] + bcL3$t[,c(17, 33)], 1, lrPerc, p = 0.5)
L50.23p = apply(bcL3$t[,1:2] + bcL3$t[,c(18, 34)], 1, lrPerc, p = 0.5)
...
( ci.L50.23p <-  quantile(L50.23p,c(0.025,0.975)) )

# Maturity size -α/βi
predicted_L50.51p <- -(coef(glm3)[1] / coef(glm3)[2])
predicted_L50.66p <- -((coef(glm3)[1] + coef(glm3)[3]) / (coef(glm3)[2] + coef(glm3)[19]))
predicted_L50.67p <- -((coef(glm3)[1] + coef(glm3)[4]) / (coef(glm3)[2] + coef(glm3)[20]))
predicted_L50.68p <- -((coef(glm3)[1] + coef(glm3)[5]) / (coef(glm3)[2] + coef(glm3)[21]))
predicted_L50.73p <- -((coef(glm3)[1] + coef(glm3)[6]) / (coef(glm3)[2] + coef(glm3)[22]))
predicted_L50.76p <- -((coef(glm3)[1] + coef(glm3)[7]) / (coef(glm3)[2] + coef(glm3)[23]))
predicted_L50.77p <- -((coef(glm3)[1] + coef(glm3)[8]) / (coef(glm3)[2] + coef(glm3)[24]))
predicted_L50.78p <- -((coef(glm3)[1] + coef(glm3)[9]) / (coef(glm3)[2] + coef(glm3)[25]))
predicted_L50.79p <- -((coef(glm3)[1] + coef(glm3)[10]) / (coef(glm3)[2] + coef(glm3)[26]))
predicted_L50.81p <- -((coef(glm3)[1] + coef(glm3)[11]) / (coef(glm3)[2] + coef(glm3)[27]))
predicted_L50.90p <- -((coef(glm3)[1] + coef(glm3)[12]) / (coef(glm3)[2] + coef(glm3)[28]))
predicted_L50.91p <- -((coef(glm3)[1] + coef(glm3)[13]) / (coef(glm3)[2] + coef(glm3)[29]))
predicted_L50.09p <- -((coef(glm3)[1] + coef(glm3)[14]) / (coef(glm3)[2] + coef(glm3)[30]))
predicted_L50.13p <- -((coef(glm3)[1] + coef(glm3)[15]) / (coef(glm3)[2] + coef(glm3)[31]))
predicted_L50.14p <- -((coef(glm3)[1] + coef(glm3)[16]) / (coef(glm3)[2] + coef(glm3)[32]))
predicted_L50.15p <- -((coef(glm3)[1] + coef(glm3)[17]) / (coef(glm3)[2] + coef(glm3)[33]))
predicted_L50.23p <- -((coef(glm3)[1] + coef(glm3)[18]) / (coef(glm3)[2] + coef(glm3)[34]))

```

```{r}

year <- c("1951", "1966", "1967", "1968", "1973", "1976", "1977", "1978", "1979", "1981", "1990", "1991", "2009", "2013", "2014", "2015", "2023")
predicted_values <- c(predicted_L50.51p, predicted_L50.66p, predicted_L50.67p, predicted_L50.68p, predicted_L50.73p, predicted_L50.76p, predicted_L50.77p, predicted_L50.78p, predicted_L50.79p, predicted_L50.81p, predicted_L50.90p, predicted_L50.91p, predicted_L50.09p, predicted_L50.13p, predicted_L50.14p, predicted_L50.15p, predicted_L50.23p)

lower_ci <- c(ci.L50.51p[1], ci.L50.66p[1], ci.L50.67p[1], ci.L50.68p[1], ci.L50.73p[1], ci.L50.76p[1], ci.L50.77p[1], ci.L50.78p[1], ci.L50.79p[1], ci.L50.81p[1], ci.L50.90p[1], ci.L50.91p[1], ci.L50.09p[1], ci.L50.13p[1], ci.L50.14p[1], ci.L50.15p[1], ci.L50.23p[1])

upper_ci <- c(ci.L50.51p[2], ci.L50.66p[2], ci.L50.67p[2], ci.L50.68p[2], ci.L50.73p[2], ci.L50.76p[2], ci.L50.77p[2], ci.L50.78p[2], ci.L50.79p[2], ci.L50.81p[2], ci.L50.90p[2], ci.L50.91p[2], ci.L50.09p[2], ci.L50.13p[2], ci.L50.14p[2], ci.L50.15p[2], ci.L50.23p[2])
...
# Excluding years (1973, 2009, 2013) which have 3 or less mature fish data
...
Y2023 <- runif(100, min=perchMat[14,3], max=perchMat[14,4])

# Combine the vectors into a data frame
data <- data.frame(Y1951, Y1966, Y1967, Y1968, Y1976, Y1977, Y1978, Y1979, Y1981, Y1990, Y1991, Y2014, Y2015, Y2023)
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -16.5, size = 5, color = "black") +
scale_x_continuous(breaks = c(1960, 1980, 2000, 2020)) +
...
# Filter the zander dataset to include only years with more than 10 observations
zander <- zander %>%
...
lmpp <- ggplot(combined_sampled_data, aes(x = year, y = predicted_values.x, group = simulation)) +
 ...
 geom_line(aes(y = predicted, color = p_value < 0.05), alpha = 0.3) +
  ...
  scale_x_continuous(breaks = c(1960, 1980, 2000)) +
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -14, size = 5, color = "black") + expand_limits(y = 0) + My_theme
```
...
# cowplot
library(grid) 
br <- br + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
r <- r + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
p <- p + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
PP <- PP + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
...
y_label <- ggdraw() + draw_label("Length at 50% maturity (cm)", angle = 90, vjust = 0.5, size = 17)

# Create a common x-axis label
x_label <- ggdraw() + draw_label("Year", vjust = 0.5, size = 17)
...
final_plot_with_x <- ggdraw() + 
  draw_grob(grid::rectGrob(gp = grid::gpar(fill = "white", col = NA)), 0, 0, 1, 1) +
  draw_plot(final_plot, 0, 0, 1, 1) +  # Adjust plot position if necessary
  draw_plot(x_label, 0, 0, 1.05, 0.001) +  # Draw x label with extra width
  theme(plot.margin = margin(t = 0, r = 10, b = 10, l = 10, unit = "mm"))

# Print the final plot with more white background
print(final_plot_with_x)

background_plot <- ggdraw() + 
  draw_plot(final_plot_with_x, x = 0.001, y = 0.001, width = 0.99, height = 0.99) +  # Adjust plot position for white space
...
ggsave("fig3.tiff", plot = background_plot, units = "mm", 
...
```{r}
PP <- lmpp + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
p <- lmp + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
br <- lmb + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
r <- lmr + labs(y = NULL, x = NULL) + theme(plot.title = element_text(size = 17, hjust = 0.5))
...
# Save the combined plot with more white space around it
ggsave("fig4.tiff", plot = background_plot, units = "mm", 
       width = 300, height = 250, dpi = 300, compression = 'lzw')
```
...
# Adding gonad stage 1 to smallest individuals which total length less than 10 and which has no gonad stage data.
Bream1 <- Bream1 %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 10, "1", gonad_stage)) %>% drop_na(gonad_stage)
...
br <- ggplot() +
  ...
  geom_line(data = plot_data, aes(x = x, y = predicted), color = "black") +  # Fitted line from model predictions
  geom_ribbon(data = plot_data, aes(x = x, ymin = ci.l, ymax = ci.u), alpha = 0.2, fill = "black") +
  annotate("text", x = Inf, y = Inf, label = paste0("p = 0.014"),
           hjust = 1.2, vjust = 2, size = 5) + labs(title = "Bream", x = "Year", y = "Length (cm)") + scale_x_continuous(breaks = specific_years) + expand_limits(y = 0) + My_theme
...
# Adding gonad stage 1 to smallest individuals which total length less than 20 and which has no gonad stage data.
Roach_mat <- Roach_mat %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 5, "1", gonad_stage)) %>% drop_na(gonad_stage)
...
# Adding gonad stage 1 to smallest individuals which total length less than 10 and which has no gonad stage data.
Perch1 <- Perch1 %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 5, "1", gonad_stage)) %>% drop_na(gonad_stage) 
...
# Adding gonad stage 1 to smallest individuals which total length less than 10 and which has no gonad stage data.
zander <- zander %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 10, "1", gonad_stage)) %>% drop_na(gonad_stage)
...
L50.51s = apply(bcL3$t[,1:2], 1, lrPerc, p = 0.5)
L50.60s = apply(bcL3$t[,1:2] + bcL3$t[,c(3, 21)], 1, lrPerc, p = 0.5)
L50.62s = apply(bcL3$t[,1:2] + bcL3$t[,c(4, 22)], 1, lrPerc, p = 0.5)
L50.66s = apply(bcL3$t[,1:2] + bcL3$t[,c(5, 23)], 1, lrPerc, p = 0.5)
L50.67s = apply(bcL3$t[,1:2] + bcL3$t[,c(6, 24)], 1, lrPerc, p = 0.5)
L50.68s = apply(bcL3$t[,1:2] + bcL3$t[,c(7, 25)], 1, lrPerc, p = 0.5)
L50.73s = apply(bcL3$t[,1:2] + bcL3$t[,c(8, 26)], 1, lrPerc, p = 0.5)
L50.77s = apply(bcL3$t[,1:2] + bcL3$t[,c(9, 27)], 1, lrPerc, p = 0.5)
L50.78s = apply(bcL3$t[,1:2] + bcL3$t[,c(10, 28)], 1, lrPerc, p = 0.5)
L50.79s = apply(bcL3$t[,1:2] + bcL3$t[,c(11, 29)], 1, lrPerc, p = 0.5)
L50.80s = apply(bcL3$t[,1:2] + bcL3$t[,c(12, 30)], 1, lrPerc, p = 0.5)
L50.81s = apply(bcL3$t[,1:2] + bcL3$t[,c(13, 31)], 1, lrPerc, p = 0.5)
L50.88s = apply(bcL3$t[,1:2] + bcL3$t[,c(14, 32)], 1, lrPerc, p = 0.5)
L50.89s = apply(bcL3$t[,1:2] + bcL3$t[,c(15, 33)], 1, lrPerc, p = 0.5)
L50.92s = apply(bcL3$t[,1:2] + bcL3$t[,c(16, 34)], 1, lrPerc, p = 0.5)
L50.96s = apply(bcL3$t[,1:2] + bcL3$t[,c(17, 35)], 1, lrPerc, p = 0.5)
L50.98s = apply(bcL3$t[,1:2] + bcL3$t[,c(18, 36)], 1, lrPerc, p = 0.5)
L50.99s = apply(bcL3$t[,1:2] + bcL3$t[,c(19, 37)], 1, lrPerc, p = 0.5)
L50.02s = apply(bcL3$t[,1:2] + bcL3$t[,c(20, 38)], 1, lrPerc, p = 0.5)
...
# Maturity size -α/βi
predicted_L50.51s <- -(coef(glm3)[1] / coef(glm3)[2])
predicted_L50.60s <- -((coef(glm3)[1] + coef(glm3)[3]) / (coef(glm3)[2] + coef(glm3)[21]))
predicted_L50.62s <- -((coef(glm3)[1] + coef(glm3)[4]) / (coef(glm3)[2] + coef(glm3)[22]))
predicted_L50.66s <- -((coef(glm3)[1] + coef(glm3)[5]) / (coef(glm3)[2] + coef(glm3)[23]))
predicted_L50.67s <- -((coef(glm3)[1] + coef(glm3)[6]) / (coef(glm3)[2] + coef(glm3)[24]))
predicted_L50.68s <- -((coef(glm3)[1] + coef(glm3)[7]) / (coef(glm3)[2] + coef(glm3)[25]))
predicted_L50.73s <- -((coef(glm3)[1] + coef(glm3)[8]) / (coef(glm3)[2] + coef(glm3)[26]))
predicted_L50.77s <- -((coef(glm3)[1] + coef(glm3)[9]) / (coef(glm3)[2] + coef(glm3)[27]))
predicted_L50.78s <- -((coef(glm3)[1] + coef(glm3)[10]) / (coef(glm3)[2] + coef(glm3)[28]))
predicted_L50.79s <- -((coef(glm3)[1] + coef(glm3)[11]) / (coef(glm3)[2] + coef(glm3)[29]))
predicted_L50.80s <- -((coef(glm3)[1] + coef(glm3)[12]) / (coef(glm3)[2] + coef(glm3)[30]))
predicted_L50.81s <- -((coef(glm3)[1] + coef(glm3)[13]) / (coef(glm3)[2] + coef(glm3)[31]))
predicted_L50.88s <- -((coef(glm3)[1] + coef(glm3)[14]) / (coef(glm3)[2] + coef(glm3)[32]))
predicted_L50.89s <- -((coef(glm3)[1] + coef(glm3)[15]) / (coef(glm3)[2] + coef(glm3)[33]))
predicted_L50.92s <- -((coef(glm3)[1] + coef(glm3)[16]) / (coef(glm3)[2] + coef(glm3)[34]))
predicted_L50.96s <- -((coef(glm3)[1] + coef(glm3)[17]) / (coef(glm3)[2] + coef(glm3)[35]))
predicted_L50.98s <- -((coef(glm3)[1] + coef(glm3)[18]) / (coef(glm3)[2] + coef(glm3)[36]))
predicted_L50.99s <- -((coef(glm3)[1] + coef(glm3)[19]) / (coef(glm3)[2] + coef(glm3)[37]))
predicted_L50.02s <- -((coef(glm3)[1] + coef(glm3)[20]) / (coef(glm3)[2] + coef(glm3)[38]))

```

```{r}
year <- c("1951", "1960", "1962", "1966", "1967", "1968", "1973", "1977", "1978", "1979", "1980", "1981", "1988", "1989", "1992", "1996", "1998", "1999", "2002")
predicted_values <- c(predicted_L50.51s, predicted_L50.60s,  predicted_L50.62s, predicted_L50.66s, predicted_L50.67s, predicted_L50.68s, predicted_L50.73s, predicted_L50.77s, predicted_L50.78s, predicted_L50.79s, predicted_L50.80s, predicted_L50.81s, predicted_L50.88s, predicted_L50.89s, predicted_L50.92s, predicted_L50.96s, predicted_L50.98s, predicted_L50.99s, predicted_L50.02s)

lower_ci <- c(ci.L50.51s[1], ci.L50.60s[1], ci.L50.62s[1], ci.L50.66s[1], ci.L50.67s[1], ci.L50.68s[1], ci.L50.73s[1], ci.L50.77s[1], ci.L50.78s[1], ci.L50.79s[1], ci.L50.80s[1], ci.L50.81s[1], ci.L50.88s[1], ci.L50.89s[1], ci.L50.92s[1], ci.L50.96s[1], ci.L50.98s[1], ci.L50.99s[1], ci.L50.02s[1])

upper_ci <- c(ci.L50.51s[2], ci.L50.60s[2], ci.L50.62s[2], ci.L50.66s[2], ci.L50.67s[2], ci.L50.68s[2], ci.L50.73s[2], ci.L50.77s[2], ci.L50.78s[2], ci.L50.79s[2], ci.L50.80s[2], ci.L50.81s[2], ci.L50.88s[2], ci.L50.89s[2], ci.L50.92s[2], ci.L50.96s[2], ci.L50.98s[2], ci.L50.99s[2], ci.L50.02s[2])
...
# Excluding years which have 1 or less mature fish data
sanderMat <- slice(sanderMatSize, -3, -4, -5, -7, -15, -19)
...
Y1968 <- runif(100, min=sanderMat[3,3], max=sanderMat[3,4])
Y1977 <- runif(100, min=sanderMat[4,3], max=sanderMat[4,4])
Y1978 <- runif(100, min=sanderMat[5,3], max=sanderMat[5,4])
Y1979 <- runif(100, min=sanderMat[6,3], max=sanderMat[6,4])
Y1980 <- runif(100, min=sanderMat[7,3], max=sanderMat[7,4])
Y1981 <- runif(100, min=sanderMat[8,3], max=sanderMat[8,4])
Y1988 <- runif(100, min=sanderMat[9,3], max=sanderMat[9,4])
Y1989 <- runif(100, min=sanderMat[10,3], max=sanderMat[10,4])
Y1996 <- runif(100, min=sanderMat[11,3], max=sanderMat[11,4])
Y1998 <- runif(100, min=sanderMat[12,3], max=sanderMat[12,4])
Y1999 <- runif(100, min=sanderMat[13,3], max=sanderMat[13,4])

# Combine the vectors into a data frame
data <- data.frame(Y1951, Y1960, Y1968, Y1977, Y1978, Y1979, Y1980, Y1981, Y1988, Y1989, Y1996, Y1998, Y1999)
...
# Save the combined plot with more white space around it
ggsave("fig3_sensitivity.tiff", plot = background_plot, units = "mm", 
       width = 300, height = 250, dpi = 300, compression = 'lzw')
...
# Save the combined plot with more white space around it
ggsave("fig4_sensitivity.tiff", plot = background_plot, units = "mm", 
       width = 300, height = 250, dpi = 300, compression = 'lzw')
```
...
df <- data.frame(
  Year = rep(seq(1950, 2020, by = 14), 4),
  Size = c(seq(55, 45, length.out = 6),   # Adult - Pop A
           seq(40, 30, length.out = 6),   # Juvenile - Pop A
           seq(35, 15, length.out = 6),   # Adult - Pop B
           seq(30, 13, length.out = 6)),  # Juvenile - Pop B
  Group = rep(c("Adult - Pop A", "Juvenile - Pop A", "Adult - Pop B", "Juvenile - Pop B"), each = 6)
)

plot1 <- ggplot(df, aes(x = Year, y = Size, color = Group)) +
  geom_line(size = 1.5) +
  ...
  scale_color_manual(values = c("#999933", "#6699CC", "#661100", "#44AA99")) +
  My_theme  +
...
# von Bertalanffy growth: Lt = Linf * (1 - exp(-k * (t - t0)))
bream_cool   <- 60 * (1 - exp(-0.3 * (age - 0)))
bream_warm   <- 54 * (1 - exp(-0.4 * (age - 0)))
roach_cold<- 49 * (1 - exp(-0.7 * (age - 0)))
roach_warm  <- 34 * (1 - exp(-0.7 * (age - 0))) 
perch_cold  <- 44 * (1 - exp(-0.7 * (age - 0))) 
perch_warm  <- 27 * (1 - exp(-0.7 * (age - 0))) 
sander_cold  <- 70 * (1 - exp(-0.7 * (age - 0))) 
sander_warm <- 140 * (1 - exp(-0.7 * (age - 0))) 

# Combine into single data frame with 3 groups
df <- data.frame(
  Age = rep(age, 3),
  Size = c(size_cool, size_warm, size_warm1),
  Temp = rep(c("Cool", "Warm", "Warm (delayed)"), each = length(age))
)

plot2 <- ggplot(df, aes(x = Age, y = Size, color = Temp, linetype = Temp)) +
  geom_line(size = 1.5) +
  scale_color_manual(values = c("Cool" = "#0072B2", 
                                "Warm" = "red", 
                                "Warm (delayed)" = "red")) +
  scale_linetype_manual(values = c("Cool" = "solid", 
                                   "Warm" = "dashed", 
                                   "Warm (delayed)" = "dashed")) +
  labs(x = "Age", y = "Size", title = " ") +
  ...
  annotate("text", x = 6.5, y = 29, label = "variable growth\nresponses to warming\namong species", size = 10) +
  My_theme +
theme(
  legend.position = "none",
  plot.title = element_text(size = 22, face = "bold"),
...
# Generate data for all species and temperature scenarios
growth_data <- data.frame(
  Age = rep(age, times = 8),
  Size = c(
    60 * (1 - exp(-0.3 * age)),    # Bream cool
    54 * (1 - exp(-0.4 * age)),    # Bream warm
    49 * (1 - exp(-0.7 * age)),    # Roach cool
    34 * (1 - exp(-0.7 * age)),    # Roach warm
    44 * (1 - exp(-0.7 * age)),    # Perch cool
    27 * (1 - exp(-0.7 * age)),    # Perch warm
    70 * (1 - exp(-0.7 * age)),    # Sander cool
    140 * (1 - exp(-0.7 * age))    # Sander warm
  ),
  Species = rep(rep(c("Bream", "Roach", "Perch", "Sander"), each = length(age)), times = 2),
  Temp = rep(c("Cool", "Warm"), each = length(age) * 4)
)

# Plot all curves together
ggplot(growth_data, aes(x = Age, y = Size, color = Species, linetype = Temp)) +
  geom_line(size = 1.4) +
  labs(x = "Age (years)", y = "Length (mm)", title = "Growth Curves by Species and Temperature") +
  scale_linetype_manual(values = c("Cool" = "solid", "Warm" = "dashed")) +
  scale_color_brewer(palette = "Dark2") +  # Or pick custom colors
  theme_minimal(base_size = 16) +
  theme(
...
# Side by side
graphic <- grid.arrange(plot1, plot2, ncol = 2)

# Save the combined plot with more white space around it
ggsave("picture.tiff", plot = graphic, units = "mm", 
       width = 400, height = 180, dpi = 300, compression = 'lzw')
```
