...
```{r}
L50data <- read_csv(file = "L50data.csv")
...
```{r}
Bream1 <- L50data %>% filter (spp_name_lt=="Karsis") %>% select(spp_name_lt, total_length, gonad_stage, year, month, sex_en) %>% filter(month == 6 | month == 7 | month == 8)

# Adding gonad stage 1 to smallest individuals which total length less than 20 and which has no gonad stage data.
Bream1 <- Bream1 %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 10, "1", gonad_stage), gonad_stage = ifelse(gonad_stage == "2" & total_length > 50, NA, gonad_stage)) %>% drop_na(gonad_stage) %>% drop_na(total_length)
...
boxplot(Bream1$total_length ~ Bream1$gonad_stage, xlab = "Gonad stage", ylab = "Length (cm)")
...
# Create a data frame with predicted L50 values
predicted_labels <- data.frame(year, predicted_values)

# Plot
ggplot(breamMatSize, aes(x = year, y = predicted_values)) +
  geom_point(size = 2, color = "black") +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.05, color = "black", size = 1) +
  geom_text(data = predicted_labels, aes(label = round(predicted_values, 1)), hjust = 1.2, vjust = -0.5, size = 4) +
  geom_smooth(method = "loess", color = "blue", size = 1, se = FALSE) +
  labs(title = "Bream Changes of L50 with Confidence Intervals",
       x = "Year",
       y = "Predicted L50") +
  ylim(15, 45) +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1),  # Increase size of x-axis text
        axis.text.y = element_text(size = 15),  # Increase size of y-axis text
...
breamMat <- breamMatSize %>% filter(predicted_values >= 0 & predicted_values <= 50)
...
# There is a highly significant negative trend in the data
...
# (Intercept) 296.50419  142.65503   2.078   0.0485 *
# year         -0.13242    0.07194  -1.841   0.0781 .
...
br <- ggplot() +
  geom_point(data = breamMat, aes(x = year, y = predicted_values), color = "#6E6E6E", size = 1.5) +  # Points for predicted_values
  ...
  annotate("text", x = Inf, y = Inf, label = paste0("p = 0.078"), 
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -18, size = 5, color = "black") +
...
```{r}
Roach_mat <- L50data %>% filter (spp_name_lt=="Kuoja") %>% select(spp_name_lt, total_length, gonad_stage, year, month, sex_en) %>% filter(month == 6 | month == 7 | month == 8)

# Adding gonad stage 1 to smallest individuals which total length less than 20 and which has no gonad stage data.
Roach_mat <- Roach_mat %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 5, "1", gonad_stage), gonad_stage = ifelse(gonad_stage == "2" & total_length > 30, NA, gonad_stage)) %>% drop_na(gonad_stage) %>% drop_na(total_length)

table(Roach_mat$gonad_stage)
table(Roach_mat$year)
```
...
```{r}
# Create box plot to see outliers
boxplot(Roach_mat$total_length ~ Roach_mat$gonad_stage, xlab = "Gonad stage", ylab = "Length (cm)")

# Identify the outliers based on the box plot
# outliers <- boxplot(Roach_mat$total_length ~ Roach_mat$gonad_stage, plot = FALSE)$out
# Remove outliers from the dataset
# Roach_mat <- Roach_mat[!(Roach_mat$total_length %in% outliers), ]
```
...
L50.50s = apply(bcL3$t[,1:2], 1, lrPerc, p = 0.5)
L50.59s = apply(bcL3$t[,1:2] + bcL3$t[,c(3, 17)], 1, lrPerc, p = 0.5)
L50.60s = apply(bcL3$t[,1:2] + bcL3$t[,c(4, 18)], 1, lrPerc, p = 0.5)
L50.62s = apply(bcL3$t[,1:2] + bcL3$t[,c(5, 19)], 1, lrPerc, p = 0.5)
L50.66s = apply(bcL3$t[,1:2] + bcL3$t[,c(6, 20)], 1, lrPerc, p = 0.5)
L50.68s = apply(bcL3$t[,1:2] + bcL3$t[,c(7, 21)], 1, lrPerc, p = 0.5)
L50.73s = apply(bcL3$t[,1:2] + bcL3$t[,c(8, 22)], 1, lrPerc, p = 0.5)
L50.78s = apply(bcL3$t[,1:2] + bcL3$t[,c(9, 23)], 1, lrPerc, p = 0.5)
L50.79s = apply(bcL3$t[,1:2] + bcL3$t[,c(10, 24)], 1, lrPerc, p = 0.5)
L50.82s = apply(bcL3$t[,1:2] + bcL3$t[,c(11, 25)], 1, lrPerc, p = 0.5)
L50.89s = apply(bcL3$t[,1:2] + bcL3$t[,c(12, 26)], 1, lrPerc, p = 0.5)
L50.90s = apply(bcL3$t[,1:2] + bcL3$t[,c(13, 27)], 1, lrPerc, p = 0.5)
L50.13s = apply(bcL3$t[,1:2] + bcL3$t[,c(14, 28)], 1, lrPerc, p = 0.5)
L50.14s = apply(bcL3$t[,1:2] + bcL3$t[,c(15, 29)], 1, lrPerc, p = 0.5)
L50.15s = apply(bcL3$t[,1:2] + bcL3$t[,c(16, 30)], 1, lrPerc, p = 0.5)
...
# Maturity size -α/βi
predicted_L50.50s <- -(coef(glm3)[1] / coef(glm3)[2])
predicted_L50.59s <- -((coef(glm3)[1] + coef(glm3)[3]) / (coef(glm3)[2] + coef(glm3)[17]))
predicted_L50.60s <- -((coef(glm3)[1] + coef(glm3)[4]) / (coef(glm3)[2] + coef(glm3)[18]))
predicted_L50.62s <- -((coef(glm3)[1] + coef(glm3)[5]) / (coef(glm3)[2] + coef(glm3)[19]))
predicted_L50.66s <- -((coef(glm3)[1] + coef(glm3)[6]) / (coef(glm3)[2] + coef(glm3)[20]))
predicted_L50.68s <- -((coef(glm3)[1] + coef(glm3)[7]) / (coef(glm3)[2] + coef(glm3)[21]))
predicted_L50.73s <- -((coef(glm3)[1] + coef(glm3)[8]) / (coef(glm3)[2] + coef(glm3)[22]))
predicted_L50.78s <- -((coef(glm3)[1] + coef(glm3)[9]) / (coef(glm3)[2] + coef(glm3)[23]))
predicted_L50.79s <- -((coef(glm3)[1] + coef(glm3)[10]) / (coef(glm3)[2] + coef(glm3)[24]))
predicted_L50.82s <- -((coef(glm3)[1] + coef(glm3)[11]) / (coef(glm3)[2] + coef(glm3)[25]))
predicted_L50.89s <- -((coef(glm3)[1] + coef(glm3)[12]) / (coef(glm3)[2] + coef(glm3)[26]))
predicted_L50.90s <- -((coef(glm3)[1] + coef(glm3)[13]) / (coef(glm3)[2] + coef(glm3)[27]))
predicted_L50.13s <- -((coef(glm3)[1] + coef(glm3)[14]) / (coef(glm3)[2] + coef(glm3)[28]))
predicted_L50.14s <- -((coef(glm3)[1] + coef(glm3)[15]) / (coef(glm3)[2] + coef(glm3)[29]))
predicted_L50.15s <- -((coef(glm3)[1] + coef(glm3)[16]) / (coef(glm3)[2] + coef(glm3)[30]))

```

```{r}
year <- c("1950", "1959", "1960", "1962", "1966", "1968", "1973", "1978", "1979", "1982", "1989", "1990", "2013", "2014", "2015")
predicted_values <- c(predicted_L50.50s, predicted_L50.59s, predicted_L50.60s, predicted_L50.62s, predicted_L50.66s, predicted_L50.68s, predicted_L50.73s, predicted_L50.78s, predicted_L50.79s, predicted_L50.82s, predicted_L50.89s, predicted_L50.90s, predicted_L50.13s, predicted_L50.14s, predicted_L50.15s)

lower_ci <- c(ci.L50.50s[1], ci.L50.59s[1], ci.L50.60s[1], ci.L50.62s[1], ci.L50.66s[1], ci.L50.68s[1], ci.L50.73s[1], ci.L50.78s[1], ci.L50.79s[1], ci.L50.82s[1], ci.L50.89s[1], ci.L50.90s[1], ci.L50.13s[1], ci.L50.14s[1], ci.L50.15s[1])

upper_ci <- c(ci.L50.50s[2], ci.L50.59s[2], ci.L50.60s[2], ci.L50.62s[2], ci.L50.66s[2], ci.L50.68s[2], ci.L50.73s[2], ci.L50.78s[2], ci.L50.79s[2], ci.L50.82s[2], ci.L50.89s[2], ci.L50.90s[2], ci.L50.13s[2], ci.L50.14s[2], ci.L50.15s[2])
...
# Plot
ggplot(roachMatSize, aes(x = year, y = predicted_values)) +
  geom_point(size = 2, color = "black") +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.05, color = "black", size = 1) +
  geom_text(data = predicted_labels, aes(label = round(predicted_values, 1)), hjust = 1.2, vjust = -0.5, size = 4) +
  geom_smooth(method = "loess", color = "blue", size = 1, se = FALSE) +
  labs(title = "Roach Changes of L50 with Confidence Intervals",
       x = "Year",
       y = "Predicted L50") +
  ylim(0, 50) +
  theme_minimal() +
...
# Remove rows where predicted_values are higher than 40 or lower than 0
roachMat <- roachMatSize %>% filter(predicted_values <= 40 & predicted_values >= 1)

roachMat2 <- select(roachMat, predicted_values)
ABtimeseries <- ts(roachMat2)
#Plot the time series data
plot(ABtimeseries)
#Add a smooth line to visualize the trend 
lines(lowess(time(ABtimeseries),ABtimeseries), col='red')
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 403.9439    62.3780   6.476 7.11e-05 ***
# year         -0.1931     0.0311  -6.210    1e-04 ***
...
annotate("text", x = max(combined_sampled_data$year), y = min(combined_sampled_data$predicted_values.x), label = paste0("p < ", significance_level, " in ", significant_p_values, "%"), hjust = 1, vjust = -18, size = 5, color = "black") + My_theme
...
```{r}
Perch1 <- L50data %>% filter (spp_name_lt=="Eserys") %>% select(spp_name_lt, total_length, gonad_stage, year, month, sex_en) %>% filter(month == 6 | month == 7 | month == 8)

# Adding gonad stage 1 to smallest individuals which total length less than 10 and which has no gonad stage data.
Perch1 <- Perch1 %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 5, "1", gonad_stage), gonad_stage = ifelse(gonad_stage == "2" & total_length > 30, NA, gonad_stage)) %>% drop_na(gonad_stage) %>% drop_na(total_length)
...
```{r}
# Create box plot to see outliers
boxplot(Perch1$total_length ~ Perch1$gonad_stage, xlab = "Gonad stage", ylab = "Length (cm)")

# no obvious outliers

# Identify the outliers based on the box plot
# outliers <- boxplot(Perch1$total_length ~ Perch1$gonad_stage, plot = FALSE)$out
# Remove outliers from the dataset
# Perch1 <- Perch1[!(Perch1$total_length %in% outliers), ]
```
...
# Plot
ggplot(perchMatSize, aes(x = year, y = predicted_values)) +
  geom_point(size = 2, color = "black") +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.05, color = "black", size = 1) +
  geom_text(data = predicted_labels, aes(label = round(predicted_values, 1)), hjust = 1.2, vjust = -0.5, size = 4) +
  labs(title = "Perch Changes of L50 with Confidence Intervals",
       x = "Year",
       y = "Predicted L50") +
  ylim(0, 40) +
  theme_minimal() +
...
# Remove rows where predicted_values are higher than 50 or lower than 1
perchMat <- perchMatSize %>% filter(predicted_values <= 50 & predicted_values >= 1)
perchMat <- slice(perchMat, -12)

# library(Kendall)
AB2 <- select(perchMat, predicted_values)
ABtimeseries <- ts(AB2)
#Plot the time series data
plot(ABtimeseries)
#Add a smooth line to visualize the trend 
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 494.90797  150.87262    3.28  0.00733 **
# year         -0.23828    0.07589   -3.14  0.00941 ** 
...
```{r}
zander <- L50data %>% filter (spp_name_lt=="Starkis") %>% select(spp_name_lt, total_length, gonad_stage, year, month, sex_en) %>% filter(month == 6 | month == 7 | month == 8)

# Adding gonad stage 1 to smallest individuals which total length less than 15 and which has no gonad stage data.
zander <- zander %>% mutate(gonad_stage = ifelse(is.na(gonad_stage) & total_length < 10, "1", gonad_stage), gonad_stage = ifelse(gonad_stage == "2" & total_length > 50, NA, gonad_stage)) %>% drop_na(gonad_stage) %>% drop_na(total_length)
...
```{r}
# Create box plot to see outliers
boxplot(zander$total_length ~ zander$gonad_stage, xlab = "Gonad stage", ylab = "Length (cm)")

# no obvious outliers

# Identify the outliers based on the box plot
# outliers <- boxplot(zander$total_length ~ zander$gonad_stage, plot = FALSE)$out
# Remove outliers from the dataset
# zander <- zander[!(zander$total_length %in% outliers), ]
```
...
L50.60s = apply(bcL3$t[,1:2] + bcL3$t[,c(3, 19)], 1, lrPerc, p = 0.5)
L50.66s = apply(bcL3$t[,1:2] + bcL3$t[,c(4, 20)], 1, lrPerc, p = 0.5)
L50.67s = apply(bcL3$t[,1:2] + bcL3$t[,c(5, 21)], 1, lrPerc, p = 0.5)
L50.68s = apply(bcL3$t[,1:2] + bcL3$t[,c(6, 22)], 1, lrPerc, p = 0.5)
L50.73s = apply(bcL3$t[,1:2] + bcL3$t[,c(7, 23)], 1, lrPerc, p = 0.5)
L50.77s = apply(bcL3$t[,1:2] + bcL3$t[,c(8, 24)], 1, lrPerc, p = 0.5)
L50.79s = apply(bcL3$t[,1:2] + bcL3$t[,c(9, 25)], 1, lrPerc, p = 0.5)
L50.80s = apply(bcL3$t[,1:2] + bcL3$t[,c(10, 26)], 1, lrPerc, p = 0.5)
L50.81s = apply(bcL3$t[,1:2] + bcL3$t[,c(11, 27)], 1, lrPerc, p = 0.5)
L50.88s = apply(bcL3$t[,1:2] + bcL3$t[,c(12, 28)], 1, lrPerc, p = 0.5)
L50.89s = apply(bcL3$t[,1:2] + bcL3$t[,c(13, 29)], 1, lrPerc, p = 0.5)
L50.92s = apply(bcL3$t[,1:2] + bcL3$t[,c(14, 30)], 1, lrPerc, p = 0.5)
L50.96s = apply(bcL3$t[,1:2] + bcL3$t[,c(15, 31)], 1, lrPerc, p = 0.5)
L50.98s = apply(bcL3$t[,1:2] + bcL3$t[,c(16, 32)], 1, lrPerc, p = 0.5)
L50.99s = apply(bcL3$t[,1:2] + bcL3$t[,c(17, 33)], 1, lrPerc, p = 0.5)
L50.02s = apply(bcL3$t[,1:2] + bcL3$t[,c(18, 34)], 1, lrPerc, p = 0.5)
...
predicted_L50.60s <- -((coef(glm3)[1] + coef(glm3)[3]) / (coef(glm3)[2] + coef(glm3)[19]))
predicted_L50.66s <- -((coef(glm3)[1] + coef(glm3)[4]) / (coef(glm3)[2] + coef(glm3)[20]))
predicted_L50.67s <- -((coef(glm3)[1] + coef(glm3)[5]) / (coef(glm3)[2] + coef(glm3)[21]))
predicted_L50.68s <- -((coef(glm3)[1] + coef(glm3)[6]) / (coef(glm3)[2] + coef(glm3)[22]))
predicted_L50.73s <- -((coef(glm3)[1] + coef(glm3)[7]) / (coef(glm3)[2] + coef(glm3)[23]))
predicted_L50.77s <- -((coef(glm3)[1] + coef(glm3)[8]) / (coef(glm3)[2] + coef(glm3)[24]))
predicted_L50.79s <- -((coef(glm3)[1] + coef(glm3)[9]) / (coef(glm3)[2] + coef(glm3)[25]))
predicted_L50.80s <- -((coef(glm3)[1] + coef(glm3)[10]) / (coef(glm3)[2] + coef(glm3)[26]))
predicted_L50.81s <- -((coef(glm3)[1] + coef(glm3)[11]) / (coef(glm3)[2] + coef(glm3)[27]))
predicted_L50.88s <- -((coef(glm3)[1] + coef(glm3)[12]) / (coef(glm3)[2] + coef(glm3)[28]))
predicted_L50.89s <- -((coef(glm3)[1] + coef(glm3)[13]) / (coef(glm3)[2] + coef(glm3)[29]))
predicted_L50.92s <- -((coef(glm3)[1] + coef(glm3)[14]) / (coef(glm3)[2] + coef(glm3)[30]))
predicted_L50.96s <- -((coef(glm3)[1] + coef(glm3)[15]) / (coef(glm3)[2] + coef(glm3)[31]))
predicted_L50.98s <- -((coef(glm3)[1] + coef(glm3)[16]) / (coef(glm3)[2] + coef(glm3)[32]))
predicted_L50.99s <- -((coef(glm3)[1] + coef(glm3)[17]) / (coef(glm3)[2] + coef(glm3)[33]))
predicted_L50.02s <- -((coef(glm3)[1] + coef(glm3)[18]) / (coef(glm3)[2] + coef(glm3)[34]))

```

```{r}

year <- c("1951", "1960", "1966", "1967", "1968", "1973", "1977", "1979", "1980", "1981", "1988", "1989", "1992", "1996", "1998", "1999", "2002")
predicted_values <- c(predicted_L50.51s, predicted_L50.60s, predicted_L50.66s, predicted_L50.67s, predicted_L50.68s, predicted_L50.73s, predicted_L50.77s, predicted_L50.79s, predicted_L50.80s, predicted_L50.81s, predicted_L50.88s, predicted_L50.89s, predicted_L50.92s, predicted_L50.96s, predicted_L50.98s, predicted_L50.99s, predicted_L50.02s)

lower_ci <- c(ci.L50.51s[1], ci.L50.60s[1], ci.L50.66s[1], ci.L50.67s[1], ci.L50.68s[1], ci.L50.73s[1], ci.L50.77s[1], ci.L50.79s[1], ci.L50.80s[1], ci.L50.81s[1], ci.L50.88s[1], ci.L50.89s[1], ci.L50.92s[1], ci.L50.96s[1], ci.L50.98s[1], ci.L50.99s[1], ci.L50.02s[1])

upper_ci <- c(ci.L50.51s[2], ci.L50.60s[2], ci.L50.66s[2], ci.L50.67s[2], ci.L50.68s[2], ci.L50.73s[2], ci.L50.77s[2], ci.L50.79s[2], ci.L50.80s[2], ci.L50.81s[2], ci.L50.88s[2], ci.L50.89s[2], ci.L50.92s[2], ci.L50.96s[2], ci.L50.98s[2], ci.L50.99s[2], ci.L50.02s[2])
...
# Plot
ggplot(sanderMatSize, aes(x = year, y = predicted_values)) +
  geom_point(size = 2, color = "black") +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.05, color = "black", size = 1) +
  geom_text(data = predicted_labels, aes(label = round(predicted_values, 1)), hjust = 1.2, vjust = -0.5, size = 4) +
  geom_smooth(method = "loess", color = "blue", size = 1, se = FALSE) +
  labs(title = "Pikeperch Changes of L50 with Confidence Intervals",
       x = "Year",
       y = "Predicted L50") +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1),  # Increase size of x-axis text
...
```{r}
# Remove rows where predicted_values are higher than 100 or lower than 1
sanderMat <- sanderMatSize %>% filter(predicted_values <= 100 & predicted_values >= 1)
# Remove values with negative ci
sanderMat <- slice(sanderMat, -4, -11, -15)
...
# Kendall's Nonparametric Test for Montonic Trend
library(EnvStats)
sanderMat$year <- as.numeric(sanderMat$year)
...
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 997.0718   236.6678   4.213  0.00179 **
# year         -0.4826     0.1191  -4.053  0.00231 **
...
PP <- base_plot +
  geom_errorbar(data = combined_data, aes(x = x, ymin = lower_ci, ymax = upper_ci), width = 0.3, size = 0.5, color = "black", alpha = 0.6) +
  annotate("text", x = Inf, y = Inf, label = paste0("p = ", format(p_value, digits = 4)), 
           hjust = 1.2, vjust = 2, size = 5)+ scale_x_continuous(breaks = specific_years) +
  My_theme
